...
import { W as _objectSpread2 } from './main-f883e670.js';
...
function createCache() {
  ...
  function get(key, fetcher) {
    ...
    var _options$DEFAULT_OPTI = _objectSpread2(_objectSpread2({}, options), DEFAULT_OPTIONS),
        ttl = _options$DEFAULT_OPTI.ttl;

    var date = new Date();
    var source = fetcher.pipe(shareReplay(CACHE_SIZE));
    ...
    var _cache$get = cache.get(key),
        keyDate = _cache$get.date,
        keyTtl = _cache$get.ttl;
...
var BASE_URL = 'https://www.speedrun.com/api/v1';
var SERIE = 'souls';
var RETRY_COUNT = 3;
/**
...
function echo(path) {
  var key = "".concat(BASE_URL).concat(path);
  return CACHE.get(key, ajax.getJSON(key).pipe(pluck('data'), retry(RETRY_COUNT)));
}
/**
...
function getLeaderboard(game, category) {
  var filters = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
  return echo("/leaderboards/".concat(game, "/category/").concat(category) + "?embed=game,category,players,variables&".concat(filters)).toPromise();
}
/**
...
function getGames() {
  return echo("/series/".concat(SERIE, "/games") + "?embed=categories,categories.game,categories.variables").toPromise();
}
/**
 * Return a run
...
function getRun(id) {
  return echo("/runs/".concat(id) + "?embed=players,game,category,category.variables,category.game").toPromise();
}

export { getLeaderboard as a, getRun as b, getGames as g };
//# sourceMappingURL=api-29f5fb02.js.map
